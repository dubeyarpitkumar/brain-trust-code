...
export default function Dashboard() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(task => task.title.toLowerCase().includes(query) || task.notes?.toLowerCase().includes(query));
    }
  ...
  return <div className="min-h-screen pb-8">
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        {loading ? <div className="space-y-4">
          ...
          </div> : filteredTasks.length === 0 ? <EmptyState /> : <AnimatePresence>
            <div className="space-y-4">
              {filteredTasks.map(task => <TaskCard key={task.id} task={task} onEdit={task => {
            setEditingTask(task);
            setFormOpen(true);
          }} onDelete={handleDeleteTask} onToggleStatus={handleToggleStatus} />)}
            </div>
          </AnimatePresence>}
      </div>

      {/* Task Form */}
      <TaskForm open={formOpen} onClose={() => {
      setFormOpen(false);
      setEditingTask(null);
    }} onSave={editingTask ? handleUpdateTask : handleCreateTask} task={editingTask} />

      {/* AI Task Generator */}
      <AITaskGenerator open={aiGeneratorOpen} onClose={() => setAiGeneratorOpen(false)} onTasksSaved={fetchTasks} />
    </div>;
}
function StatCard({
  title,
